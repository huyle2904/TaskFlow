...
import { boardApi, taskApi, groupApi, userApi } from '../api/services';
import type { TaskBoard, TaskItem, TaskItemStatus, TaskPriority, UserDto } from '../types';
...
export default function BoardDetailPage() {
  ...
  const [formAssignedToIds, setFormAssignedToIds] = useState<string[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (boardRes.data.groupId) {
        const membersRes = await groupApi.getMembers(boardRes.data.groupId);
        const memberUsers: UserDto[] = membersRes.data.map(m => ({
          id: m.userId,
          fullName: m.userName,
          email: m.email
        }));
      ...
      toast.error(error?.response?.data?.message || 'Không thể tải dữ liệu board');
    } finally {
      setIsLoading(false);
    }
  }, [boardId]);
  ...
  const openEditModal = (task: TaskItem) => {
    ...
    setShowCreateModal(true);
    setFormAssignedToIds(task.assignedToIds || []);
    setFormIsPrivate(task.isPrivate || false);
  };
    ...
    try {
      if (editingTask) {
        await taskApi.update(boardId, editingTask.id, {
          ...
          assignedToIds: formAssignedToIds.length > 0 ? formAssignedToIds : undefined,
          status: formStatus,
      ...
      toast.error(error?.response?.data?.message || (editingTask ? 'Cập nhật thất bại' : 'Tạo task thất bại'));
    } finally {
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmitTask} className="space-y-4">
              ...
              {/* Assign to multiple users with checkboxes */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Giao cho</label>
                <div className="max-h-40 overflow-y-auto border border-gray-200 rounded-lg p-2 space-y-1">
                  {users.length === 0 ? (
                    <p className="text-sm text-gray-400">Không có thành viên nào</p>
                  ) : (
                    users.map((user) => (
                      <label key={user.id} className="flex items-center gap-2 p-1 hover:bg-gray-50 rounded cursor-pointer">
                        <input
                          type="checkbox"
                          checked={formAssignedToIds.includes(user.id)}
                          onChange={(e) => {
                            if (e.target.checked) {
                              setFormAssignedToIds([...formAssignedToIds, user.id]);
                            } else {
                              setFormAssignedToIds(formAssignedToIds.filter(id => id !== user.id));
                            }
                          }}
              ...
              <div className="col-span-2 flex items-center gap-2 mt-2">
                ...
                <label htmlFor="isPrivate" className="text-sm text-gray-700">
                  Task riêng tư (chỉ owner và người được giao mới xem được)
                </label>
              </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {(task.assignedToIds && task.assignedToIds.length > 0) ? (
            <div className="flex flex-wrap gap-1">
              {task.assignedToIds.map((userId) => {
                const user = users.find(u => u.id === userId);
                return user ? (
                  <span key={userId} className="text-xs text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">
                    {user.fullName}
                  </span>
                ) : null;
              })}
            </div>
          ) : task.assignedToId && users.find(u => u.id === task.assignedToId) ? (
            <span className="text-xs text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">
              {users.find(u => u.id === task.assignedToId)?.fullName}
            </span>
          ) : null}
        </div>
