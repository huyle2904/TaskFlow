...
using TaskFlow.Application.Features.Boards.Queries.GetBoardsByGroup;
...
public class BoardsController : ControllerBase
{
    ...
    {
        var command = new CreateBoardCommand(dto.Name, dto.Description, GetUserId(), dto.GroupId);
    ...
    [ProducesResponseType(typeof(List<TaskBoardDto>), StatusCodes.Status200OK)]
    public async Task<ActionResult<List<TaskBoardDto>>> GetBoardsByGroup(Guid groupId)
    {
        var query = new GetBoardsByGroupQuery { GroupId = groupId, UserId = GetUserId() };
        var result = await _mediator.Send(query);
        return Ok(result);
    }
