...
// 4. HTTPS Redirection - chuyển HTTP → HTTPS (bỏ ở dev để tránh lỗi)
...
{
    var context = scope.ServiceProvider.GetRequiredService<TaskFlow.Infrastructure.Data.AppDbContext>();
...
try
{
    using (var scope = app.Services.CreateScope())
    {
        ...
        await TaskFlow.Infrastructure.SeedData.SeedData.SeedAsync(context);
