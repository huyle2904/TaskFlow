...
public class TaskBoardRepository : GenericRepository<TaskBoard>, ITaskBoardRepository
{
    ...
    public async Task<IEnumerable<TaskBoard>> GetByGroupIdsAsync(List<Guid> groupIds)
    {
        return await _dbSet
            .AsNoTracking()
            .Include(b => b.Tasks)
            .Where(b => b.GroupId.HasValue && groupIds.Contains(b.GroupId.Value))
            .OrderByDescending(b => b.CreatedAt)
            .ToListAsync();
    }

    public async Task<IEnumerable<TaskBoard>> GetByGroupIdAsync(Guid groupId)
    {
        return await _dbSet
            .AsNoTracking()
            .Include(b => b.Tasks)
